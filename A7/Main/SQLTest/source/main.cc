			...
			if (pos != string :: npos) {
				...
				// if we did parse correctly, just print out the query
					...
					if (final->isCreateTable ()) {
						...
						clock_t tic = clock();
						auto res=myPlan->execute();
						clock_t toc = clock();
						MyDB_RecordPtr temp = res->getEmptyRecord ();
						MyDB_RecordIteratorAltPtr myIter = res->getIteratorAlt ();
						int cnt=0;
						...
						printf("Cnt: %d, Cost: %f seconds\n",cnt, (double)(toc - tic) / CLOCKS_PER_SEC);
